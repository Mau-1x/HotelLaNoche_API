...
// FORZAR IPv4 EN TODO EL PROCESO
...
async function sendMail(to, subject, htmlContent) {
    try {
        ...
        console.log(`Email '${subject}' enviado con éxito a ${to}`);
    } catch (e) {
        console.error("ERROR EN BREVO:", e.response ? JSON.stringify(e.response.data) : e.message);
    }
}
    ...
    try {
        ...
        const html = `
            <div style="background-color: #0F172A; padding: 40px; font-family: sans-serif; color: #F8FAFC; text-align: center; border-radius: 20px; border: 2px solid #2563EB;">
                <h1 style="color: #60A5FA;">ACTIVACIÓN DE CUENTA</h1>
                <p>Bienvenido <strong>${nombres}</strong>. Su código de seguridad es:</p>
                <div style="background: #1E293B; padding: 20px; border-radius: 12px; margin: 20px auto; width: fit-content; border: 1px dashed #60A5FA;">
                    <span style="font-size: 42px; font-weight: bold; letter-spacing: 10px; color: #FFFFFF;">${otp}</span>
                </div>
            </div>`;

        await sendMail(email, "👑 Active su Membresía VIP - Hotel La Noche", html);
...
app.post('/api/forgot-password', async (req, res) => {
    ...
    try {
        let pool = await sql.connect(dbConfig);
        // Guardamos el código de recuperación en la columna codigo_verificacion temporalmente
        const result = await pool.request()
            .input('mail', sql.VarChar, email)
            .input('otp', sql.VarChar, recoveryCode)
            .query(`
                UPDATE CLIENTE
                SET codigo_verificacion = @otp
                WHERE id_persona = (SELECT id_persona FROM PERSONA WHERE email = @mail)
            `);

        if (result.rowsAffected[0] > 0) {
            const html = `
                <div style="background-color: #0F172A; padding: 40px; font-family: sans-serif; color: #F8FAFC; text-align: center; border-radius: 20px; border: 2px solid #EF4444;">
                    <h1 style="color: #F8FAFC;">RECUPERACIÓN DE CUENTA</h1>
                    <p>Ha solicitado restablecer su contraseña. Utilice este código:</p>
                    <div style="background: #1E293B; padding: 20px; border-radius: 12px; margin: 20px auto; width: fit-content; border: 1px dashed #EF4444;">
                        <span style="font-size: 42px; font-weight: bold; letter-spacing: 10px; color: #FFFFFF;">${recoveryCode}</span>
                    </div>
                    <p style="font-size: 12px; color: #94A3B8;">Si no solicitó este cambio, ignore este mensaje.</p>
                </div>`;
            await sendMail(email, "🔑 Recuperación de Contraseña - Hotel La Noche", html);
...
app.post('/api/reset-password', async (req, res) => {
    ...
    try {
        ...
        const result = await pool.request()
            ...
            .input('pass', sql.VarChar, newPassword)
            .query(`
                UPDATE CLIENTE
                SET contrasena = @pass, codigo_verificacion = NULL
                WHERE id_persona = (SELECT id_persona FROM PERSONA WHERE email = @mail)
        ...
        if (result.rowsAffected[0] > 0) {
            ...
            res.status(400).json({ error: 'El código de seguridad es incorrecto' });
        }
    } catch (err) { res.status(500).json({ error: err.message }); }
});
